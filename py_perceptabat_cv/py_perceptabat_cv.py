...
def split_file(filepath: str, chunksize: int = 5000) -> None:
    ...
    if line_count == 0:
        os.remove(os.path.abspath(chunked_file_object.name))
...
def run_cmd(flag_list: List[str] = [], verbose: bool = False) -> None:
    ...
    output = subprocess.run(cmd_list, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).stdout
    if verbose or b'Total structures: 1       Processed: 1' not in output:
        print(output.decode('utf-8')    )

def parse_percepta_output(result_file: object, offset: int = 0) -> Dict[str, Dict[str, str]]:
...
def py_perceptabat_cv(percepta_cmd_str: str, threads: Optional[int] = None) -> Dict[str, Dict[str, str]]:
    ...
    if 'TFNAME' not in percepta_cmd_str or '-R' in percepta_cmd_str or '-F' in percepta_cmd_str:
        raise ValueError('Input/output file must be a .TXT file. .SDF and .RDF extensions are not supported.')
    ...
    if '__' in i_filename or ' ' in i_filename or 'chunk' in i_filename:
        raise ValueError("Please do not use '__' or space or 'chunk' characters/words in input file name.")
    ...
    try:
        ...
        for l in (line for line in open(abs_input_filepath, 'r')):
            if len(l.split(' ')) != 2: # checking the format of input file; does not check for SMILES validity
    ...
    for chunk in split_file_list:
        chunk_no = int(chunk.split('__chunk__')[1].split('.')[0])
        percepta_cmd_str_chunk = percepta_cmd_str.replace(i_filename, chunk).replace(
            o_filename, o_filename + '__chunk__result' + str(chunk_no))
    ...
    trans_result_dict = write_csv(result_dict, translation_dict,
        input_filepath=abs_input_filepath, output_filepath=os.path.join(i_dirname, o_filename))
